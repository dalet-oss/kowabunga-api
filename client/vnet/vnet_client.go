...
/*
GetVNet Returns a description of the virtual network
*/
func (a *Client) GetVNet(params *GetVNetParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*GetVNetOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewGetVNetParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "GetVNet",
		Method:             "GET",
		...
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &GetVNetReader{formats: a.formats},
		AuthInfo:           authInfo,
		Context:            params.Context,
		Client:             params.HTTPClient,
	}
	...
	success, ok := result.(*GetVNetOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for GetVNet: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
