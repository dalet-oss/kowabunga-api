...
func (a *Client) GetPoolVolumes(params *GetPoolVolumesParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*GetPoolVolumesOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewGetPoolVolumesParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "GetPoolVolumes",
		Method:             "GET",
		PathPattern:        "/pool/{poolId}/volumes",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &GetPoolVolumesReader{formats: a.formats},
		AuthInfo:           authInfo,
		Context:            params.Context,
		Client:             params.HTTPClient,
	}
	...
	success, ok := result.(*GetPoolVolumesOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for GetPoolVolumes: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
